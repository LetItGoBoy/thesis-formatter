  ...
  return (
    <main className="min-h-screen bg-[#eef1f8] text-slate-900">
      ...
      <section className="mx-auto max-w-6xl px-5 pt-4">
        ...
        <p className="mt-4 max-w-2xl text-base leading-8 text-slate-500">{meta.intro}</p>
        <div className="mt-5 flex flex-wrap items-center gap-3 text-sm text-slate-500">
          <span className="rounded-full bg-slate-50 px-3 py-1 ring-1 ring-slate-200">
            案件进度 {solvedCount} / {levels.length}
          </span>
          {allSolved && (
            <span className="rounded-full bg-emerald-500/15 px-3 py-1 text-emerald-600 ring-1 ring-emerald-500/30">
              线索集齐，可以指认了
            </span>
          )}
        </div>
        {loadError && (
          <div className="mt-4 rounded-lg bg-red-500/10 px-4 py-2 text-sm text-red-600 ring-1 ring-red-500/20">
            数据库加载失败：{loadError}
          </div>
        )}
      </section>
          ...
          {showBrief && (
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              {caseData.briefing.map((b) => (
                <div key={b.term} className="rounded-xl bg-slate-50 p-4 ring-1 ring-slate-200">
                  <div className="text-sm font-semibold text-indigo-700">{b.term}</div>
                  <p className="mt-1.5 text-sm leading-6 text-slate-600">{b.story}</p>
                  <div className="mt-2 rounded-lg bg-slate-50 px-3 py-2 text-xs leading-5 text-slate-500">
                    <span className="font-semibold text-slate-600">定义　</span>
                    {b.definition}
                  </div>
                </div>
              ))}
            ...
            <div className="mt-4 grid gap-5 md:grid-cols-2">
              <div>
                ...
                <ol className="mt-2 space-y-2 text-sm leading-6 text-slate-600">
                  <li>
                    <span className="mr-1.5 font-semibold text-emerald-600">1.</span>
                    你是查档侦探，所有线索都藏在右侧「案件数据库」的表里。
                  </li>
                  <li>
                    <span className="mr-1.5 font-semibold text-emerald-600">2.</span>
                    在黑色框里写一句 <span className="font-mono text-emerald-600">SQL</span>，点「运行查询」就能向数据库提问、看结果。
                  </li>
                  <li>
                    <span className="mr-1.5 font-semibold text-emerald-600">3.</span>
              ...
              <div>
                ...
                <div className="mt-2 space-y-1.5 font-mono text-[12px] leading-5">
                  {[
                    ...
                    <div key={code} className="rounded-md bg-slate-100 px-3 py-1.5 ring-1 ring-slate-200">
                      <div className="text-[11px] text-slate-500">{label}</div>
                      <code className="text-emerald-700">{code}</code>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <section className="mx-auto grid max-w-6xl gap-6 px-5 py-8 lg:grid-cols-[1fr_19rem]">
          ...
          {!accuseActive ? (
            <>
              <div className="flex items-center gap-2 text-sm font-semibold text-emerald-600">
                ...
                <span className="text-slate-500">知识点 · {level.concept}</span>
              </div>
              <h2 className="mt-3 text-2xl font-semibold">{level.title}</h2>
              <p className="mt-3 leading-7 text-slate-600">{level.story}</p>

              <div className="mt-5 rounded-xl bg-amber-400/5 px-4 py-3 ring-1 ring-amber-400/20">
                <div className="text-sm font-semibold text-amber-600">🔎 查询任务</div>
                <p className="mt-1 text-sm leading-6 text-slate-600">{level.task}</p>
              </div>
              ...
              {feedback === "hit" && (
                <div className="mt-4 flex items-center gap-2 rounded-xl bg-emerald-500/10 px-4 py-3 text-sm text-emerald-600 ring-1 ring-emerald-500/20">
              ...
              <div className="mt-4 flex items-center gap-4">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 text-sm text-slate-500 transition hover:text-amber-600"
                >
                  <Lightbulb size={15} />
              ...
              {showHint && (
                <div className="mt-2 space-y-2">
                  {/* 第一层：文字思路 */}
                  <div className="rounded-lg bg-slate-50 px-4 py-3 text-sm leading-6 text-slate-600 ring-1 ring-slate-200">
                    💡 {level.hint}
                  </div>
                  {/* 第二层：语句骨架（再点一下才显示，输入框本身不提示） */}
                  <details className="rounded-lg bg-slate-50 px-4 py-3 ring-1 ring-slate-200">
                    <summary className="cursor-pointer text-sm text-slate-500 transition hover:text-amber-600">
                      还不会写？看语句骨架（把空格 ___ 填上）
                    </summary>
                    <pre className="mt-2 overflow-auto rounded bg-slate-100 px-3 py-2 font-mono text-xs text-emerald-700">
                      {level.skeleton}
                    </pre>
                  </details>
                </div>
            ...
            <>
              ...
              <p className="mt-3 leading-7 text-slate-600">{accusation.prompt}</p>
              ...
              {accuseState !== "win" ? (
                <div className="mt-6 max-w-sm">
                  <input
                    ...
                    placeholder="输入凶手姓名"
                    className="w-full rounded-xl bg-slate-100 px-4 py-3 text-slate-900 ring-1 ring-slate-200 placeholder:text-slate-500 focus:outline-none focus:ring-emerald-500/50"
                  />
                  {accuseState === "wrong" && (
                  ...
                  <button
                    ...
                    className="mt-4 inline-flex items-center gap-2 rounded-full bg-rose-500 px-5 py-2.5 text-sm font-semibold text-slate-900 transition hover:bg-rose-400"
                  >
                    <Fingerprint size={16} />
                    正式指认
                  </button>
                ...
                <div className="mt-6 rounded-2xl bg-emerald-500/10 p-6 ring-1 ring-emerald-500/30">
                  ...
                  <p className="mt-2 text-sm leading-7 text-slate-600">{accusation.winNote}</p>
                  <div className="mt-4 flex flex-wrap gap-3">
                    ...
                    <button
                      type="button"
                      onClick={restart}
                      className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-medium text-slate-800 transition hover:bg-slate-200"
                    >
                      <RotateCcw size={15} />
                      重玩本案
                    </button>
        ...
        <aside className="space-y-5">
          <div className="rounded-2xl bg-white p-5 ring-1 ring-slate-200">
            <div className="text-sm font-semibold text-slate-600">侦破步骤</div>
          ...
          <div className="rounded-2xl bg-white p-5 ring-1 ring-slate-200">
            ...
            <div className="mt-3 space-y-3">
              {schema.map((t) => (
                <div key={t.name} className="rounded-lg bg-slate-100 p-3 ring-1 ring-slate-200">
                  <button
                    type="button"
                    onClick={() => previewTable(t.name)}
                    title={`预览 ${t.name} 的全部数据`}
                    className="font-mono text-[13px] text-emerald-600 underline-offset-2 hover:underline"
                  >
                    {t.name}
                  </button>
