  ...
  return (
    <main className="min-h-screen bg-[#0f1320] text-slate-100">
      ...
      <section className="mx-auto max-w-5xl px-5 pt-4">
        ...
        <p className="mt-4 max-w-2xl text-base leading-8 text-slate-400">{SEASON_META.villain}</p>
      ...
      <section className="mx-auto max-w-5xl px-5 py-8">
            ...
            return (
              <button
                ...
                className={`group flex flex-col items-start gap-2 rounded-2xl p-5 text-left ring-1 transition ${
                  ready
                    ? "bg-[#161b2c] ring-white/10 hover:ring-emerald-500/40"
                    : "cursor-not-allowed bg-[#121626] ring-white/5"
                }`}
              >
                <div className="flex w-full items-center justify-between">
                  <span
                    className={`grid size-8 place-items-center rounded-lg text-sm font-bold ${
                      ready ? "bg-emerald-500/15 text-emerald-300" : "bg-white/5 text-slate-600"
                    }`}
                  >
                    {entry.order}
                  </span>
                  {ready ? (
                    <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-2.5 py-0.5 text-xs font-medium text-emerald-300 ring-1 ring-emerald-500/20">
                      可进入
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1 rounded-full bg-white/5 px-2.5 py-0.5 text-xs text-slate-500">
                      <Lock size={11} />
                      敬请期待
                    </span>
                  )}
                </div>
                <div className={`mt-1 text-lg font-semibold ${ready ? "text-slate-100" : "text-slate-500"}`}>
                  第{entry.order}案 · {entry.title}
                </div>
                <div className="text-xs text-slate-500">{entry.concepts}</div>
                {ready && (
                  <span className="mt-2 inline-flex items-center gap-1 text-sm font-medium text-emerald-300 opacity-0 transition group-hover:opacity-100">
        ...
        <div className="mt-8 flex items-start gap-2 rounded-xl bg-white/5 px-4 py-3 text-sm text-slate-400 ring-1 ring-white/10">
          <Fingerprint size={16} className="mt-0.5 shrink-0 text-emerald-300" />
